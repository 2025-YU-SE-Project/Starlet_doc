...
- 밤하늘 페이지에서 별을 선택 후 'Genarate' 버튼을 눌렀을 때 7~14개의 별을 선택하지 않았다면 해당 모달이 나타난다.
...
- 마지막 선 지우기 버튼과 전체 삭제 버튼을 통해 선을 연결한 것을 삭제할 수 있다.
...
- 하단 입력란에 별자리 이름과 간단한 설명을 작성 후 ‘완료’ 버튼을 누르면 별자리 생성이 완료된다.  
- 별자리 이름은 10자 이내, 설명은 30자 이내로 작성하도록 제한된다.
- 상단 좌측의 '<' 버튼으로 별자리 선 잇기 모달로 돌아갈 수 있다.
- ? 튤팁을 호버하면 AI 추천 기능에 대한 설명을 볼 수 있다.
- 추천받기 버튼을 누르면 입력란에 이름과 설명을 추천해서 나타난다.
...
- 생성한 별자리의 모양 그대로 정해진 일정한 크기로 나타난다.  
...
- 슬라이드 위의 원을 위로 올리면 별자리 크기가 커지며 내리면 크기가 줄어들고 이 후 슬라이드 하단의 체크 아이콘을 누르면 적용된다.
- 드래그 시에도 드래그 후 체크 아이콘을 누르면 저장된다.
- 드래그 또는 크기 조절 후 체크 아이콘을 누르지 않고 바탕화면 클릭 시 원래 위치로 돌아간다.
...
- 별자리 드래그 및 크기 조절 후 체크 아이콘을 누르면 해당 모달이 나타난다.
...
- 보고자 하는 별자리 목록을 클릭하면 상세 페이지로 이동한다.  
- 생성일자, 이름, 설명, 감정 구성을 기본적으로 확인할 수 있다.  
- 내가 남긴 기록보기에서 생성 날짜를 클릭하면 해당 날짜의 일기를 보여준다.
...
- 별자리 이름과 설명을 수정할 수 있고 '완료' 버튼을 누르면 수정사항이 저장된다.
...
- 친구들의 기록된 별과 생성한 별자리 수, 레벨에 따른 호칭을 확인할 수 있다.
...
- 친구 신청이 완료되면 '친구 신청' 버튼이 신청 완료로 바뀌며 버튼이 비활성화된다.

---

![그림: 친구 검색(이미 친구입니다)](./User%20interface%20prototype%20image/image-friend-search2.png)

- 이미 친구를 맺은 친구의 이름을 입력 후 돋보기 버튼을 누르면 "이미 친구입니다"라는 문구가 나타나며 버튼이 비활성화된다.
...
- '수락'을 누르면 친구가 맺어지며 '거절'을 누르면 친구 요청이 거부된다.
